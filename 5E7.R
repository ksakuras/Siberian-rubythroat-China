...
raw.5E7 <- read.csv("data/0_PAM/5E7_act_pres_temp.csv")

# change timestamp back to correct format
raw.5E7$timestamp = as.POSIXct(raw.5E7$timestamp,tz = "GMT", format = c("%Y-%m-%d %H:%M:%S"))
head(raw.5E7)
#CHECK whether it is GMT!
unique(raw.5E7$timestamp)
summary(raw.5E7)
str(raw.5E7)

# extract info
pressure.5E7.mid = subset(raw.5E7, series == "pressure")
acto.5E7.mid = subset(raw.5E7, series == "actoscore")
temp.5E7.mid = subset(raw.5E7, series == "temperature")


# combine all information into pam variable
pam.5E7 <- list(length = 4)  #create a list variable
pam.5E7$pressure <- data.frame(pressure.5E7.mid[,c(2,3)])  #copy the column 2-3 to the list
pam.5E7$acceleration <- data.frame(acto.5E7.mid[,c(2,3)])
pam.5E7$temperature <- data.frame(temp.5E7.mid[,c(2,3)])
pam.5E7$light <- data.frame(acto.5E7.mid[,c(2,3)])   #create a list of light with acto schedule
pam.5E7$light[,2] <- rep(0,length(pam.5E7$light[,2])) #force all data to 0 as we don't have light data
names(pam.5E7) <- c("id","pressure","acceleration","temperature","light") #change "length" list to name "id"
pam.5E7$id <- as.character("5E7")
colnames(pam.5E7$pressure) <- c("date","obs")  #change column name for each list
colnames(pam.5E7$acceleration) <- c("date","obs")
colnames(pam.5E7$light) <- c("date","obs")
colnames(pam.5E7$temperature) <- c("date","obs")

summary(pam.5E7)
str(pam.5E7)

#now the format is compatible, we can follow the manual!
trainset_write(pam.5E7,pathname = "data/1_pressure/labels/")  #auto-generate a .csv file
pam.5E7 <- trainset_read(pam.5E7, pathname = "data/1_pressure/labels/") #auto-find the -labeled.csv file
pam.5E7 <- pam_sta(pam.5E7)
knitr::kable(head(pam.5E7$sta))
str(pam.5E7)

pressure_na <- pam.5E7$pressure
pressure_na$obs[pressure_na$isoutlier | pressure_na$sta_id == 0] <- NA
p <- ggplot() +
  geom_line(data = pam.5E7$pressure, aes(x = date, y = obs), col = "grey") +
  geom_line(data = pressure_na, aes(x = date, y = obs, col = as.factor(sta_id))) +
  geom_point(data = subset(pam.5E7$pressure, isoutlier), aes(x = date, y = obs), colour = "black") +
  theme_bw() +
...
for (i_r in seq_len(length(pressure_prob))) {
  ...
  info <- pam.5E7$sta[pam.5E7$sta$sta_id == i_s, ]
...
pressure_timeserie_1 <- geopressure_ts(pt$lon, pt$lat, pressure = subset(pam.5E7$pressure, sta_id == 1))
...
col <- rep(RColorBrewer::brewer.pal(9, "Set1"), times = ceiling((nrow(pam.5E7$sta) + 1) / 9))
col <- col[1:(nrow(pam.5E7$sta) + 1)]
names(col) <- levels(factor(c(0, pam.5E7$sta$sta_id)))

p <- ggplot() +
  geom_line(data = pam.5E7$pressure, aes(x = date, y = obs), colour = "grey") +
  geom_point(data = subset(pam.5E7$pressure, isoutlier), aes(x = date, y = obs), colour = "black") +
...
save(
  ...
  pressure_prob,
  pam,
  file = "data/1_pressure/5E7_pressure_prob.Rdata"
)
