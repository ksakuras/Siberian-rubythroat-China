...
raw.56C <- read.csv("data/0_PAM/56C_act_pres_temp.csv")

# change timestamp back to correct format
raw.56C$timestamp = as.POSIXct(raw.56C$timestamp,tz = "GMT", format = c("%Y-%m-%d %H:%M:%S"))
head(raw.56C)
#CHECK whether it is GMT!
unique(raw.56C$timestamp)
summary(raw.56C)
str(raw.56C)

# extract info
pressure.56C.mid = subset(raw.56C, series == "pressure")
acto.56C.mid = subset(raw.56C, series == "actoscore")
temp.56C.mid = subset(raw.56C, series == "temperature")


# combine all information into pam variable
pam.56C <- list(length = 4)  #create a list variable
pam.56C$pressure <- data.frame(pressure.56C.mid[,c(2,3)])  #copy the column 2-3 to the list
pam.56C$acceleration <- data.frame(acto.56C.mid[,c(2,3)])
pam.56C$temperature <- data.frame(temp.56C.mid[,c(2,3)])
pam.56C$light <- data.frame(acto.56C.mid[,c(2,3)])   #create a list of light with acto schedule
pam.56C$light[,2] <- rep(0,length(pam.56C$light[,2])) #force all data to 0 as we don't have light data
names(pam.56C) <- c("id","pressure","acceleration","temperature","light") #change "length" list to name "id"
pam.56C$id <- as.character("56C")
colnames(pam.56C$pressure) <- c("date","obs")  #change column name for each list
colnames(pam.56C$acceleration) <- c("date","obs")
colnames(pam.56C$light) <- c("date","obs")
colnames(pam.56C$temperature) <- c("date","obs")

summary(pam.56C)
str(pam.56C)

#now the format is compatible, we can follow the manual!
trainset_write(pam.56C,pathname = "data/1_pressure/labels/")  #auto-generate a .csv file
pam.56C <- trainset_read(pam.56C, pathname = "data/1_pressure/labels/") #auto-find the -labeled.csv file
pam.56C <- pam_sta(pam.56C)
knitr::kable(head(pam.56C$sta))
str(pam.56C)

pressure_na <- pam.56C$pressure
pressure_na$obs[pressure_na$isoutlier | pressure_na$sta_id == 0] <- NA
p <- ggplot() +
  geom_line(data = pam.56C$pressure, aes(x = date, y = obs), col = "grey") +
  geom_line(data = pressure_na, aes(x = date, y = obs, col = as.factor(sta_id))) +
  geom_point(data = subset(pam.56C$pressure, isoutlier), aes(x = date, y = obs), colour = "black") +
  theme_bw() +
...
for (i_r in seq_len(length(pressure_prob))) {
  ...
  info <- pam.56C$sta[pam.56C$sta$sta_id == i_s, ]
...
pressure_timeserie_1 <- geopressure_ts(pt$lon, pt$lat, pressure = subset(pam.56C$pressure, sta_id == 1))
...
col <- rep(RColorBrewer::brewer.pal(9, "Set1"), times = ceiling((nrow(pam.56C$sta) + 1) / 9))
col <- col[1:(nrow(pam.56C$sta) + 1)]
names(col) <- levels(factor(c(0, pam.56C$sta$sta_id)))

p <- ggplot() +
  geom_line(data = pam.56C$pressure, aes(x = date, y = obs), colour = "grey") +
  geom_point(data = subset(pam.56C$pressure, isoutlier), aes(x = date, y = obs), colour = "black") +
...
save(
  ...
  pressure_prob,
  pam,
  file = "data/1_pressure/56C_pressure_prob.Rdata"
)
