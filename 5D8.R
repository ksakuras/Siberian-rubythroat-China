...
raw.5D8 <- read.csv("data/0_PAM/5D8_act_pres_temp.csv")

# change timestamp back to correct format
raw.5D8$timestamp = as.POSIXct(raw.5D8$timestamp,tz = "GMT", format = c("%Y-%m-%d %H:%M:%S"))
head(raw.5D8)
#CHECK whether it is GMT!
unique(raw.5D8$timestamp)
summary(raw.5D8)
str(raw.5D8)

# extract info
pressure.5D8.mid = subset(raw.5D8, series == "pressure")
acto.5D8.mid = subset(raw.5D8, series == "actoscore")
temp.5D8.mid = subset(raw.5D8, series == "temperature")


# combine all information into pam variable
pam.5D8 <- list(length = 4)  #create a list variable
pam.5D8$pressure <- data.frame(pressure.5D8.mid[,c(2,3)])  #copy the column 2-3 to the list
pam.5D8$acceleration <- data.frame(acto.5D8.mid[,c(2,3)])
pam.5D8$temperature <- data.frame(temp.5D8.mid[,c(2,3)])
pam.5D8$light <- data.frame(acto.5D8.mid[,c(2,3)])   #create a list of light with acto schedule
pam.5D8$light[,2] <- rep(0,length(pam.5D8$light[,2])) #force all data to 0 as we don't have light data
names(pam.5D8) <- c("id","pressure","acceleration","temperature","light") #change "length" list to name "id"
pam.5D8$id <- as.character("5D8")
colnames(pam.5D8$pressure) <- c("date","obs")  #change column name for each list
colnames(pam.5D8$acceleration) <- c("date","obs")
colnames(pam.5D8$light) <- c("date","obs")
colnames(pam.5D8$temperature) <- c("date","obs")

summary(pam.5D8)
str(pam.5D8)

#now the format is compatible, we can follow the manual!
trainset_write(pam.5D8,pathname = "data/1_pressure/labels/")  #auto-generate a .csv file
pam.5D8 <- trainset_read(pam.5D8, pathname = "data/1_pressure/labels/") #auto-find the -labeled.csv file
pam.5D8 <- pam_sta(pam.5D8)
knitr::kable(head(pam.5D8$sta))
str(pam.5D8)

pressure_na <- pam.5D8$pressure
pressure_na$obs[pressure_na$isoutlier | pressure_na$sta_id == 0] <- NA
p <- ggplot() +
  geom_line(data = pam.5D8$pressure, aes(x = date, y = obs), col = "grey") +
  geom_line(data = pressure_na, aes(x = date, y = obs, col = as.factor(sta_id))) +
  geom_point(data = subset(pam.5D8$pressure, isoutlier), aes(x = date, y = obs), colour = "black") +
  theme_bw() +
...
for (i_r in seq_len(length(pressure_prob))) {
  ...
  info <- pam.5D8$sta[pam.5D8$sta$sta_id == i_s, ]
...
pressure_timeserie_1 <- geopressure_ts(pt$lon, pt$lat, pressure = subset(pam.5D8$pressure, sta_id == 1))
...
col <- rep(RColorBrewer::brewer.pal(9, "Set1"), times = ceiling((nrow(pam.5D8$sta) + 1) / 9))
col <- col[1:(nrow(pam.5D8$sta) + 1)]
names(col) <- levels(factor(c(0, pam.5D8$sta$sta_id)))

p <- ggplot() +
  geom_line(data = pam.5D8$pressure, aes(x = date, y = obs), colour = "grey") +
  geom_point(data = subset(pam.5D8$pressure, isoutlier), aes(x = date, y = obs), colour = "black") +
...
save(
  ...
  pressure_prob,
  pam,
  file = "data/1_pressure/5D8_pressure_prob.Rdata"
)
