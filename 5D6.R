...
trainset_write(pam.5D6,pathname = "data/1_pressure/labels/")  #auto-generate a .csv file
pam.5D6 <- trainset_read(pam.5D6, pathname = "data/1_pressure/labels/") #auto-find the -labeled.csv file
pam.5D6 <- pam_sta(pam.5D6)
knitr::kable(head(pam.5D6$sta))
str(pam.5D6)

pressure_na <- pam.5D6$pressure
pressure_na$obs[pressure_na$isoutlier | pressure_na$sta_id == 0] <- NA
p <- ggplot() +
  geom_line(data = pam.5D6$pressure, aes(x = date, y = obs), col = "grey") +
  geom_line(data = pressure_na, aes(x = date, y = obs, col = as.factor(sta_id))) +
  geom_point(data = subset(pam.5D6$pressure, isoutlier), aes(x = date, y = obs), colour = "black") +
  theme_bw() +
...
pressure_maps <- geopressure_map(
  pam.5D6$pressure,
  extent = c(43, 90, 5, 116), # coordinates of the map to request (N, W, S, E)
...
for (i_r in seq_len(length(pressure_prob))) {
  ...
  info <- pam.5D6$sta[pam.5D6$sta$sta_id == i_s, ]
...
pressure_timeserie_1 <- geopressure_ts(pt$lon, pt$lat, pressure = subset(pam.5D6$pressure, sta_id == 1))
...
ggplotly(p, dynamicTicks = T) %>% layout(showlegend = F)  #why starting from 2017?

col <- rep(RColorBrewer::brewer.pal(9, "Set1"), times = ceiling((nrow(pam.5D6$sta) + 1) / 9))
col <- col[1:(nrow(pam.5D6$sta) + 1)]
names(col) <- levels(factor(c(0, pam.5D6$sta$sta_id)))

p <- ggplot() +
  geom_line(data = pam.5D6$pressure, aes(x = date, y = obs), colour = "grey") +
  geom_point(data = subset(pam.5D6$pressure, isoutlier), aes(x = date, y = obs), colour = "black") +
...
save(
  ...
  pressure_prob,
  pam,
  file = "data/1_pressure/5D6_pressure_prob.Rdata"
)
